...
const CheckBoxRefTask = () => {
  ...
  const printLanguage = () =>{
    ...
    // inputRef.current.forEach((data) => console.log(data.checked))
    let comLanguage = ""
    inputRef.current.forEach((data, index, arr) =>{
      console.log(data.value)
      if(data.checked){
        comLanguage += index === arr.length - 1 ? data.value : data.value + ", "
      }
    })
    setResult(comLanguage)
  }
  return (
    <div>
      <p>{result}</p>
      <label>
        <span>JAVA</span>
        <input onChange={printLanguage} type="checkbox" name="language" ref={(el) => inputRef.current[0] = el} value="JAVA"/>
      </label>
      <label>
        <span>Python</span>
        <input onChange={printLanguage} type="checkbox" name="language" ref={(el) => inputRef.current[1] = el} value="Python"/>
      </label>
      <label>
        <span>JS</span>
        <input onChange={printLanguage} type="checkbox" name="language" ref={(el) => inputRef.current[2] = el} value="JS"/>
      </label>
    </div>
  );
};
